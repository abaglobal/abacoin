Abacoin Core version *0.15.1* is now available from:

  <https://download.abacoin.org/abacoin-0.15.1/>
...
A number of changes to the way Abacoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 avas per kB (0.00001 ABAG). This is to prepare for dropping the minimum transaction fee to 0.1 avas per kB (0.0001 ABAG) in the next release.
