...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 2664
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 2665
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 12664
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 12665
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 2664
	%{_sbindir}/semanage port -d -p tcp 2665
	%{_sbindir}/semanage port -d -p tcp 12664
	%{_sbindir}/semanage port -d -p tcp 12665
	for selinuxvariant in %{selinux_variants}; do
